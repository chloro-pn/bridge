...
unique_ptr<Object> CoroutineScheduler::Parse() {
  ...
  if (NeedToSplit() == false) {
    NormalScheduler ns(content_, parse_ref_);
    return ns.Parse();
  }
...
unique_ptr<Object> Parse(const std::string& content, ParseOption po) {
  if (po.type == SchedulerType::Normal) {
    NormalScheduler ns(content, po.parse_ref);
    return ns.Parse();
  }
  if (po.type == SchedulerType::Coroutine) {
    CoroutineScheduler cs(content, po.parse_ref, po.worker_num_);
    return cs.Parse();
  }
